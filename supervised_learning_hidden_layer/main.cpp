...
plot_network_separation_lines(const NeuralNetwork &network)
{
    ...
    for (size_t j = 0; j < network.num_hidden; ++j) {
        ...
        weight_line_x *= (1.0 / length) * LINE_LENGTH;
        weight_line_y *= (1.0 / length) * LINE_LENGTH;

        double y_intersect = threshold / weight_normal_y;

        auto line_x = {-weight_line_x, weight_line_x};
        auto line_y = {-weight_line_y + y_intersect, weight_line_y + y_intersect};
...
main()
{
    ...
    const size_t ENERGY_EVERY_X_STEP = 1000;
    ...
    std::vector<double> test_iter_vec;               test_iter_vec.resize(NUM_TRAINING_ITERATIONS / ENERGY_EVERY_X_STEP);
    std::vector<double> test_training_energy_vec;    test_training_energy_vec.resize(NUM_TRAINING_ITERATIONS / ENERGY_EVERY_X_STEP);
    std::vector<double> test_validation_energy_vec;  test_validation_energy_vec.resize(NUM_TRAINING_ITERATIONS / ENERGY_EVERY_X_STEP);
    ...
    for (size_t test = 0; test < NUM_TESTS; ++test) {
        ...
        for (size_t training_iteration = 0; training_iteration < NUM_TRAINING_ITERATIONS; ++training_iteration) {
            ...
            if (training_iteration % ENERGY_EVERY_X_STEP == 0) {
                ...
                size_t result_index = training_iteration / ENERGY_EVERY_X_STEP;
                test_iter_vec[result_index] = training_iteration;
                test_training_energy_vec[result_index] = training_energy;
                test_validation_energy_vec[result_index] = validation_energy;
            }
        }
    ...
    plt::subplot(1, 2, 2);
    {
        ...
        plot_input_data(training_data, validation_data);
        plot_network_separation_lines(*network);
        plt::xlim(-2.0, +2.0);
        plt::ylim(-2.1, +2.1);
        plt::legend();
        plt::grid(true);
