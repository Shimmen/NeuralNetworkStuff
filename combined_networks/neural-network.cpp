...
void
NeuralNetwork::reset_weights(std::default_random_engine rng, double min, double max)
{
    std::uniform_real_distribution<double> double_in_range(min, max);

    for (size_t i = 0; i < output_weights.num_elements; ++i) {
        output_weights.set_linear(i, double_in_range(rng));
    }
}

void
NeuralNetwork::reset_thresholds(std::default_random_engine rng, double min, double max)
{
    std::uniform_real_distribution<double> double_in_range(min, max);
    
    for (size_t i = 0; i < output_thresholds.size(); ++i) {
        output_thresholds[i] = double_in_range(rng);
    }
}
...
void
NeuralNetwork::train(const std::vector<double>& training_input, const std::vector<double>& expected_output, double learning_rate)
{
    ...
    std::vector<double> delta_thresholds(num_outputs);
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
        ...
        double delta_threshold = learning_rate * (expected_out[i] - output_data[i]) * -activation_function_derivative(local_field);
